...
class Server(object):
	...
	def render(self, gif, text=''):
		...
		#with tempfile.NamedTemporaryFile(suffix='.gif', prefix='render_', dir=self.gif_dir, mode='a+b', delete=False) as gif_f:
		with open(os.path.join(self.gif_dir, 'render_%d_%s.gif' % (subtitle, text_hash)), 'a+b') as gif_f:
			gif_f.seek(0)
